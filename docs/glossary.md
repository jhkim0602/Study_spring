...
여러 코드에 반복되는 공통 기능을 핵심 로직에서 분리해서,
필요한 메서드 실행 지점에 자동으로 적용하는 방식이다.
...
해당 클래스나 메서드가 원래 처리해야 하는 주된 업무다.
...
AOP는 이 공통 관심사를 별도 Aspect로 분리한다.

### Aspect

Advice와 Pointcut을 묶은 AOP 모듈이다.
...
Advice를 적용할 수 있는 위치다.

Spring AOP에서는 주로 스프링 빈의 메서드 실행 시점이 JoinPoint다.

### Pointcut

Advice를 어디에 적용할지 고르는 조건이다.
...
이 표현식은 `Lect_B.week07` 하위의 서비스 메서드를 대상으로 삼는다.

### `@Before`

대상 메서드가 실행되기 전에 실행되는 Advice다.

주로 인증, 권한 확인, 파라미터 검증에 사용한다.

### `@AfterReturning`

대상 메서드가 예외 없이 정상 종료된 뒤 실행되는 Advice다.

반환값을 확인하거나 성공 로그를 남길 때 사용한다.

### `@AfterThrowing`

대상 메서드에서 예외가 발생했을 때 실행되는 Advice다.

예외 로그나 알림 처리에 사용한다.

### `@After`

대상 메서드 실행 후 항상 실행되는 Advice다.

정상 종료와 예외 발생에 관계없이 실행된다는 점에서 `finally`와 비슷하다.

### `@Around`

대상 메서드 실행 전후 전체를 감싸는 Advice다.

`ProceedingJoinPoint.proceed()`를 호출해야 실제 대상 메서드가 실행된다.

### `ProceedingJoinPoint`

`@Around` Advice에서 사용하는 JoinPoint 타입이다.

대상 메서드 실행을 직접 진행하거나, 인자를 바꿔 실행하거나, 반환값을 가공할 수 있다.

### `execution`

메서드 실행 모양을 기준으로 Pointcut 대상을 고르는 표현식이다.

예:

```java
execution(* Lect_B.week07..*.*(..))
```

### `within`

특정 클래스나 패키지 안의 메서드를 대상으로 고르는 표현식이다.

### `args`

메서드 인자 타입을 기준으로 Pointcut 대상을 고르는 표현식이다.

### `@annotation`

특정 어노테이션이 붙은 메서드를 대상으로 고르는 표현식이다.
...
즉 스프링 빈의 메서드 실행 흐름에 공통 기능을 적용하는 방법을 배운다.
